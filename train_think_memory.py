...
SAVE_PATH = os.path.expanduser("~/teamwork_projects/recurrent_model_test/smollm_think_memory.pt")

def train_think_memory():
    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print("=" * 65)
    print(f"  TRENING PAMIĘCI Z TOKENAMI MYŚLENIA NA SMOLLM2-135M ({device.upper()})")
    print(f"  Scena -> [4 Sloty] + Pytanie + [4 Tokeny Myślenia] -> Odpowiedź")
    print("=" * 65)
    ...
    print(f">>> Trenowalne parametry (Sloty + Tokeny Myślenia): {trainable_params / 1e3:.1f}k ({trainable_params * 2 / 1024:.1f} KB)\n")
    ...
    # Optymalizator ze schedulerem
    optimizer = torch.optim.AdamW(memory.bridge.parameters(), lr=1.5e-3, weight_decay=0.01)

    print("=" * 65)
    print("  ROZPOCZĘCIE TRENINGU (10 EPOK Z TOKENAMI MYŚLENIA)")
    print("=" * 65)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (passage, question, answer) in enumerate(train_data, 1):
            ...
            # Krok 2: Odpowiedź przez 4 tokeny myślenia
            ...
            if i % 50 == 0 or i == len(train_data):
                ...
                print(f"  [Epoka {epoch + 1:02d}/{num_epochs:02d}] Krok {i:03d}/{len(train_data)} | Loss: {loss.item():.4f} | Gate: {gate_val:.4f}")
