...
WEIGHTS_PATH = os.path.expanduser("~/teamwork_projects/recurrent_model_test/smollm_scene_slots.pt")

def evaluate_slots(num_tests: int = 15):
    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print("=" * 65)
    print(f"  BENCHMARK: DYNAMIC SCENE SLOTS NA SMOLLM2-135M ({device.upper()})")
    print(f"  Scena (Tura 1) -> [4 Sloty Pamięci (4.6 KB)] -> Pytanie (Tura 2)")
    print("=" * 65)
    ...
    test_data = generate_dataset(num_samples=num_tests, base_seed=1234)
    hits = 0
    total = len(test_data)
    ...
    for i, (passage, question, expected_answer) in enumerate(test_data, 1):
        ...
        # 2. Odpowiedź z samych slotów (zero tekstu z Tury 1)
        generated_answer = memory.answer_from_scene(scene_vectors, question)
